#!/usr/bin/env python3
"""
Controlled hyperparameter study: Random Forest min_samples_leaf.

This study changes exactly one modeling decision at a time: the RandomForest
`min_samples_leaf` (1, 2, 4, 8, 16). Everything else stays fixed:
  ...
  - training and validation molecules only (the standard-scaffold TEST set
    is never touched);
  - n_estimators = 300, random_state = 42, n_jobs = -1, and all other RF
    parameters at their current defaults.

Primary model-selection metric: validation Spearman. Secondary: validation
RMSE / MAE / R2. Training metrics and generalization gaps are diagnostic only.
Tree complexity is reported from the fitted estimators so the relationship

    min_samples_leaf -> tree complexity -> training fit -> validation
    generalization
...
N_ESTIMATORS = 300
SEED = 42
MIN_SAMPLES_LEAF_LIST = [1, 2, 4, 8, 16]


def make_rf(min_samples_leaf: int) -> RandomForestRegressor:
    """RF with the M2/M3 reference settings, only the leaf size changed."""
...
def forest_complexity(
    rf: RandomForestRegressor, X_train: np.ndarray
) -> dict:
    ...
    # Empirical mean number of training molecules per leaf per tree.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="RandomForest min_samples_leaf study (validation only)"
    )
    parser.add_argument(
    ...
    for msl in MIN_SAMPLES_LEAF_LIST:
        print(f"\n--- min_samples_leaf={msl} ---", flush=True)
        rf = make_rf(msl)
        ...
        complexity = forest_complexity(rf, X_train)
    ...
    csv_path = results_dir / "min_samples_leaf_tuning_results.csv"
    out.to_csv(csv_path, index=False)

    summary = {
        "milestone": "RandomForest min_samples_leaf tuning study",
        ...
        "min_samples_leaf_values": MIN_SAMPLES_LEAF_LIST,
    ...
    json_path = results_dir / "min_samples_leaf_tuning_results.json"
    ...
    # Figure 1: train vs validation metrics, making the gap visible.
    msl_vals = [r["min_samples_leaf"] for r in rows]
    fig, axes = plt.subplots(1, 2, figsize=(9, 4))
    for ax, metric in zip(axes, ["Spearman", "R2"]):
        ax.plot(
            msl_vals,
            [r[f"train_{metric}"] for r in rows],
            marker="o",
            label="train",
            color="#C44E52",
        )
        ...
        ax.legend()
    fig.tight_layout()
    fig1_path = figures_dir / "min_samples_leaf_train_val.png"
    fig.savefig(fig1_path, dpi=150)
    plt.close(fig)

    # Figure 2: validation metrics and tree complexity together.
    fig, axes = plt.subplots(1, 2, figsize=(9, 4))
    axes[0].plot(
        msl_vals,
    ...
    axes[0].set_title("Validation R2 / Spearman")
    axes[0].grid(alpha=0.3)
    axes[0].legend()

    axes[1].plot(
        msl_vals,
        [r["mean_tree_depth"] for r in rows],
        marker="o",
        label="mean depth",
        color="#8172B2",
    )
    axes[1].set_xlabel("min_samples_leaf")
    axes[1].set_ylabel("mean tree depth")
    axes[1].set_title("Tree complexity")
    axes[1].grid(alpha=0.3)
    axes[1].legend()
    fig.tight_layout()
    fig2_path = figures_dir / "min_samples_leaf_complexity.png"
